...
//   autoMarket, autoGalaxyMarket, autoFactory, and autoMiningDroid use weightings and priorities to determine their tasks. Resources split by groups of same priority, and then resources within group having the best priority
//   distributed according to their weights. If there's still some more unused routes\factories\drones after assigning, script moves to next group with lower priority, etc. In most cases only one group with highest priority is
//   active and working, while other groups serve as fallback for cases when all resources with better priority are either capped, or, in case of factory, unaffordable. There's few special values for finer configuration:
...
(function ($) {
  ...
  var SmelterManager = {
    ...
    Productions: normalizeProperties({
      ...
      Steel: {
        id: "Steel", unlocked: () => resources.Steel.isUnlocked() && haveTech("smelting", 2), resource: resources.Steel,
  ...
  var FactoryManager = {
    ...
    Productions: addProps(normalizeProperties({
      LuxuryGoods: {
        id: "Lux", resource: resources.Money, unlocked: () => true,
        cost: [new ResourceProductionCost(resources.Furs, () => FactoryManager.f_rate("Lux", "fur"), 5)]
      },
      Furs: {
        id: "Furs", resource: resources.Furs, unlocked: () => haveTech("synthetic_fur"),
      ...
      Alloy: {
        id: "Alloy", resource: resources.Alloy, unlocked: () => true,
      ...
      Polymer: {
        id: "Polymer", resource: resources.Polymer, unlocked: () => haveTech("polymer"),
      ...
      NanoTube: {
        id: "Nano", resource: resources.Nano_Tube, unlocked: () => haveTech("nano"),
      ...
      Stanene: {
        id: "Stanene", resource: resources.Stanene, unlocked: () => haveTech("stanene"),
  ...
  var TriggerManager = {
    ...
    actionConflicts(trigger) {
      if (settings.allowCompetingTriggers) {
        return this.targetTriggers.some(t => t.actionId == trigger.actionId);
      }

      // Only checks if two triggers use the same resource, it does not check storage
  ...
  var GameLog = {
    ...
    getTimeString() {
      ...
      return `[${hour}:${minute}:${second}] [${game.global.stats.days} days] `;
    },
    ...
    logInfo(loggingType, text, tags) {
      ...
      poly.messageQueue(GameLog.getTimeString() + text, "info", false, tags);
    },
    ...
    logSuccess(loggingType, text, tags) {
      ...
      poly.messageQueue(GameLog.getTimeString() + text, "success", false, tags);
    },
    ...
    logWarning(loggingType, text, tags) {
      ...
      poly.messageQueue(GameLog.getTimeString() + text, "warning", false, tags);
    },
    ...
    logDanger(loggingType, text, tags) {
      ...
      poly.messageQueue(GameLog.getTimeString() + text, "danger", false, tags);
    },
  }
  ...
  const argType = {
    ...
    project: {
      def: "arpalaunch_facility", arg: "select_cb", options: () => Object.values(arpaIds).map(p =>
        ({ val: p._vueBinding, label: p.name }))
    },
    job: {
      def: "unemployed", arg: "select_cb", options: () => Object.values(jobIds).map(j =>
        ({ val: j._originalId, label: j._originalName }))
    },
    job_servant: {
      def: "farmer", arg: "select_cb", options: () => Object.values(jobIds).filter(j => j.is.serve).map(j =>
        ({ val: j._originalId, label: j._originalName }))
    },
    resource: {
      def: "Food", arg: "select_cb", options: () => Object.values(resources).map(r =>
        ({ val: r._id, label: r.name }))
    },
    race: {
      def: "species", arg: "select_cb", options: () =>
    ...
    challenge: {
      def: "junker", arg: "select_cb", options: () => challenges.flat().map(c =>
        ({ val: c.trait, label: game.loc(`evo_challenge_${c.id}`), hint: game.loc(`evo_challenge_${c.id}_effect`) }))
    },
    universe: {
      def: "standard", arg: "select_cb", options: () =>
    ...
    soldiers: {
      def: "workers", arg: "select_cb", options: () =>
    ...
    tab: {
      def: "civTabs1", arg: "select_cb", options: () =>
  ...
  const checkTypes = {
    ...
    CountriesUnified: { fn: () => Object.values(game.global.civic.foreign).filter(({ anx, buy, occ }) => anx || buy || occ).length, arg: "string", def: "__returns_number__", desc: "Returns the number of unified countries." },
  ...
  function updateUI() {
    ...
    if (scriptNode.length === 0) {
      ...
      createSettingToggle(togglesNode, 'allowCompetingTriggers', 'Allows triggers requiring competing resources.');
